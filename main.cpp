...
const std::string MODEL_PATH = "trueno.obj";
...
private:
    ...
    void initWindow() {
        ...
        window = glfwCreateWindow(WIDTH, HEIGHT, "KTJST!", nullptr, nullptr);
        delq.push([=](){
    ...
    void createTextureImage() {
        ...
        stbi_uc* pixels = stbi_load("ktjst.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
    ...
    // TODO: Take this into use at some point and test reszing window.
    ...
    void mainLoop() {
        ...
        vkDeviceWaitIdle(device); // Wait for GPU to be ready for our program to die?
    }
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "KTJST!";
        appInfo.applicationVersion = VK_MAKE_VERSION(1, 3, 3);
        ...
        extensions.push_back(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME); // Mac stuff
    ...
    void pickPhysicalDevice() {
        ...
        if (physicalDevice == VK_NULL_HANDLE) {
            throw std::runtime_error("did not find ok gpu");
        }
    }
    ...
    void createGraphicsPipeline() {
        ...
        VkPushConstantRange frameInfoPushConstant;
        frameInfoPushConstant.offset = 0;
        frameInfoPushConstant.size = sizeof(FrameInfo);
        frameInfoPushConstant.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
        pipelineLayoutInfo.pPushConstantRanges = &frameInfoPushConstant;
    ...
    void recordCommandBuffer(VkCommandBuffer cmds, uint32_t imageIndex) {
        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        ...
        clearColor.color.float32[0] = (sin(frameCount * 0.002) + 1) * 0.5;
        clearColor.color.float32[1] = (sin(frameCount * 0.005) + 1) * 0.5;
        clearColor.color.float32[2] = (cos(frameCount * 0.01) + 1) * 0.5;
        ...
        vkCmdBeginRenderPass(cmds, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
            vkCmdBindPipeline(cmds, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline);
            ...
            vkCmdBindIndexBuffer(cmds, indexBuffer, 0, VK_INDEX_TYPE_UINT32);
            ...
            vkCmdPushConstants(cmds, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(FrameInfo), &frameInfo);

        vkCmdBindDescriptorSets(cmds, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets[currentFrame], 0, nullptr);
        vkCmdDrawIndexed(cmds, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        // I did not know this kind of magic was possible
        ...
        ubo.model = glm::rotate(glm::mat4(1.0f), time * 0.2f * glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        ubo.proj = glm::perspective(glm::radians(100.0f), swapChainExtent.width / (float) swapChainExtent.height, 0.1f, 10.0f);
        ubo.proj[1][1] *= -1; // flip because y inverted
    ...
    bool isDeviceSuitable(VkPhysicalDevice device) {
        ...
        return indices.isOk() && extensionsSupported && swapChainAdequate && supportedFeatures.samplerAnisotropy;
    }
